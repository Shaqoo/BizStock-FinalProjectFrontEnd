...
let totalPages = 1;
const prevPageBtn = document.getElementById("brand-prev-page");
const nextPageBtn = document.getElementById("brand-next-page");
const pageInfo = document.getElementById("brand-page-info");
const productList = document.getElementById("brand-products-results");
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  currentQuery = id;  
  console.log("Current brand ID:", currentQuery);
  fetchProducts();
});
...
async function fetchProducts() {
  try {
    let url = `${apiBaseUrl}/Products/brand/${currentQuery}?Page=${currentPage}&PageSize=${pageSize}`;
