...
function buildMessageDiv(msg, msgDate) {
  ...
  if (!isMe) {
    ...
    div.innerHTML = `
      ...
      <div class="bg-white p-3 rounded-2xl shadow max-w-xs message-bubble">
        ${msg.message ? `<p class="text-sm text-black">${msg.message}</p>` : ""}
        ${msg.pictureUrl ? `<img src="${msg.pictureUrl}" class="mt-2 rounded-lg max-w-[200px]">` : ""}
        ${
          msg.audioUrl
            ? `
          <div class="flex items-center space-x-2 mt-2">
            <button class="play-btn text-black w-8 h-8 flex items-center justify-center">▶</button>
            <div id="waveform-${msg.id}" class="flex-1"></div>
            <span id="duration-${msg.id}" class="text-xs text-black"></span>
          </div>
        `
            : ""
        }
        <span class="text-xs text-black block mt-1">${"Customer Service Officer"} • ${time}</span>
      </div>
    `;
  } else {
  ...
  msg.reactions.forEach(r => {
    ...
    emojiEl.title = r.userId == getCurrentUserId() ? "You" : "Customer Service Officer";
